...
void GraphicsView::mouseReleaseEvent(QMouseEvent *)
{
    ...
    if (item.count() > 0) {
        ...
        if (!intersectedRect.isEmpty()) {
            QString fn = QFileDialog::getSaveFileName(this, tr("Save"), last_file_name, tr("jpeg file(*.jpg *.jpeg);;png file(*.png);;tiff file(*.tiff);;bmp file(*.bmp)"));
            if (!fn.isEmpty()) {
...
void GraphicsView::SetLastFileName(QString fn)
{
    int x=0;
    int i = fn.lastIndexOf(".");
    QString qs1=fn.mid(0,i);
    int n=qs1.length();
    for(int j=n-1;j>=0;j--)
    {
        if(qs1[j]>='0'&&qs1[j]<='9')
    ...
    QString qs=fn.mid(0,i-x);
    QString tt=QString::number( qs1.mid(i-x,x).toInt()+1 );
    last_file_name=qs+tt;
}
